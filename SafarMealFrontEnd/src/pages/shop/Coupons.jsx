...
import { Gift, Trash2, Plus, Calendar, AlertCircle, Tag, Sparkles } from 'lucide-react';
...
const Coupons = () => {
  ...
  const fetchOffers = async () => {
    try {
      const { data } = await api.get('/shop/offers');
      if (data.success) {
        setOffers(data.data);
      }
    } catch (err) {
  ...
  const handleCreateCoupon = async (e) => {
    ...
    try {
      ...
      const { data } = await api.post('/shop/offers', payload);
      if (data.success) {
  ...
  return (
    <div className="space-y-8 font-inter text-slate-100 w-full max-w-6xl mx-auto">
      {/* Title */}
      <div>
        <h1 className="font-poppins font-black text-2xl sm:text-3xl text-white flex items-center gap-2">
          <Gift className="text-brand-500 animate-pulse" /> Restaurant Coupons & Offers
        </h1>
        <p className="text-xs text-slate-400 font-semibold mt-1">
          Create and publish custom discount coupons. Customers will see these as banners and copy them.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="font-poppins font-bold text-sm text-white flex items-center gap-2 mb-2">
            <Sparkles size={16} className="text-brand-500" /> Active Promo Campaign ({offers.length})
          </h3>

          {offers.length === 0 ? (
            <div className="p-12 bg-slate-900/40 border border-slate-900 rounded-3xl text-center">
              <Gift size={48} className="mx-auto text-slate-700 mb-3" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {offers.map((off) => {
                ...
                return (
                  <div
                    key={off._id}
                    className={`bg-slate-900/40 border ${isExpired ? 'border-slate-900 opacity-60' : 'border-slate-850'} p-5 rounded-3xl shadow-premium relative overflow-hidden flex flex-col justify-between space-y-4 hover:border-slate-700 transition`}
                  >
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="px-3 py-1 bg-brand-950/80 text-brand-400 border border-brand-900/40 rounded-xl text-xs font-black tracking-wider flex items-center gap-1.5 uppercase">
                    ...
                    <div className="border-t border-slate-905/60 pt-3 flex flex-wrap gap-x-4 gap-y-2 text-[10px] font-semibold text-slate-400 relative z-10">
                      <div>
                        <span>Max Cap: </span>
                      ...
                      <div className="flex items-center gap-1">
                        <Calendar size={10} />
                        <span className={isExpired ? 'text-red-400 font-bold' : 'text-slate-450'}>
        ...
        <div className="lg:col-span-1 bg-slate-900/40 border border-slate-900 p-6 sm:p-8 rounded-3xl shadow-premium h-fit">
          <h3 className="font-poppins font-bold text-sm text-white flex items-center gap-2 mb-4">
            <Plus size={16} className="text-brand-500" /> Create Promo Coupon
          </h3>
          ...
          <form onSubmit={handleCreateCoupon} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="text"
                required
                placeholder="e.g. PIZZA50, BURGER25"
                value={code}
                onChange={(e) => setCode(e.target.value)}
                className="w-full text-xs font-semibold px-4 py-3 bg-slate-950 border border-slate-800 text-white rounded-xl focus:outline-none focus:border-brand-500 uppercase"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full text-xs font-semibold px-4 py-3 bg-slate-950 border border-slate-800 text-slate-350 rounded-xl focus:outline-none focus:border-brand-500"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              className="w-full py-3.5 bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 text-white font-bold text-xs rounded-xl shadow-glow transition mt-2 flex items-center justify-center gap-1.5"
            >
